...
| -------- | ------------------------------------------------------------ |
| 适用阶段 | MVP 首版上线                                                 |
| 变更重点 | 冻结 UserRuntimeBinding / 任务状态机 /browserUrl/internalEndpoint、禁用与删除语义 |
| 本版原则 | 先上线、后演进；能并行开发、能稳定联调、能支持首版发布       |

**边界声明**：本契约覆盖 MVP 必需能力：能登录、能识别用户、能拉起 runtime、能展示模型、能托管最小凭据、能展示 usage summary、能做最小治理。复杂多租户、复杂共享空间和复杂费用管理不纳入本版基线。
...
| MVP 目标   | 登录、用户同步、runtime 启停、模型列表、最小凭据托管闭环、usage summary、管理后台最小治理、用户工作台最小可用。 |
| 统一限制   | 每用户最多 1 个 runtime；角色只保留 user/admin；用户状态只保留 active/disabled。 |
| MVP 非目标 | 复杂多租户、复杂共享空间 UI、复杂套餐和费用结算、复杂策略中心。 |
...
| browserUrl        | 浏览器访问 runtime 的地址，由 Traefik 子域名暴露。           |
...
| credential.status | active / invalid / disabled。                                |
| binding.source    | platform_default / user_owned。                              |
...
- 模块 2 是用户、状态、quota、UserRuntimeBinding 的业务真相层。
- 模块 3 读取模块 2 的 binding，读取模块 4 的模型配置，渲染 config/secret，再调用 runtime manager。
- 模块 4 提供模型列表、凭据元数据、内部 gateway-config 和 usage 归集。
- 模块 5 仅给 admin 使用，消费模块 2 与模块 4 的治理数据。
- 模块 6 仅给当前用户使用，消费模块 1/3/4 的用户侧能力。

| 模块                       | 职责                                                         |
| -------------------------- | ------------------------------------------------------------ |
| 模块 1：身份与访问接入     | 对接 Authentik、登录态校验、首次登录触发 /internal/users/sync、输出 AuthContext、识别 admin/disabled。 |
| 模块 2：租户与用户资源控制 | 维护 User/Tenant/Quota/UserRuntimeBinding；不碰 Docker。     |
| 模块 3：Runtime 编排       | 提供 ensure_running /stop/delete /inspect；生成启动模板；调用 runtime manager；回写状态。 |
| 模块 4：模型接入与凭据代理 | 返回模型列表；托管用户凭据；内部生成 gateway-config；归集 usage summary。 |
| 模块 5：管理后台           | 用户治理、runtime 查看、模型开关、凭据状态查看、usage summary。 |
| 模块 6：用户工作台         | 展示用户信息、runtime 状态、模型和凭据、usage summary、工作区入口。 |
...
| runtime 启动        | 模块 3 必须先渲染模型配置文件和 secret file，再调用 runtime manager 挂载。 |
| gateway-config 权限 | 仅 internal 可读，不向普通用户前端暴露。                     |
| 禁用收口            | 禁用用户后立即 desiredState=stopped，并使已签发 token 立即失效。 |
| 删除语义            | 删除 runtime 时必须显式携带 retentionPolicy；MVP 默认 preserve_workspace。 |
| 本地 profile        | runtime 保留本地 profile，但首次进入由平台自动创建并映射。   |
| 用量口径            | MVP 展示口径以 OpenClaw 上报为主；后续审计与计费以网关日志为准。 |
...
- 已禁用用户可识别身份，但业务接口统一返回 403 USER_DISABLED。
- 首次登录自动触发用户同步。
...
### 6.4 模块 4：模型接入、凭据代理与用量归集

- 平台默认模型对新用户开箱即用。
- 用户可管理自己的 provider 凭据，但不可读取明文 key。
...
- 用户可管理自己的凭据并查看 usage summary。

## 7. 统一联调主流程

1. 首次登录：Authentik 登录 -> 模块 1 调 /internal/users/sync -> 模块 2 幂等创建用户 -> 模块 6 获取 /auth/me。
2. 启动 runtime：模块 6 调 /users/me/runtime/start -> 模块 3 读取 binding 和  model config -> 渲染 config/secret -> 调 runtime manager  ensure-running -> 模块 2 回写 binding。
3. 进入工作区：模块 6 调 /workspace-entry -> ready=true 且 browserUrl 非空 -> 前端跳转。
4. 禁用用户：模块 5 改状态 -> 模块 2 置为 disabled -> 编排层收敛 desiredState=stopped -> token 立即失效。
