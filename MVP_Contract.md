...
| -------- | ------------------------------------------------------------ |
| 适用阶段 | MVP 首版上线                                                 |
| 变更重点 | 冻结 UserRuntimeBinding / 任务状态机 / browserUrl / internalEndpoint、禁用与删除语义，并补齐首次 binding 初始化边界与 workspace 前置鉴权模型 |
...
- MVP 以查看、启停开关和状态治理为主。
- 不展示复杂费用看板，不做财务结算。
- 后台最小接口集应覆盖用户列表、用户详情、用户状态修改、runtime 详情、凭据状态与 usage summary。
...
- 首页或任务轮询场景优先消费 runtime 轻量状态接口；配置、详情与删除确认场景消费完整 binding 接口。
...
4. 禁用用户：模块 5 改状态 -> 模块 2 置为 `disabled` -> 编排层收敛 `desiredState=stopped` -> token 立即失效 -> 后续业务访问统一返回 `403 USER_DISABLED`。
...
| 接口一致性 | 前端只依赖统一接口完成首页、后台、runtime 控制、模型和凭据展示；`/users/me/runtime` 与 `/users/me/runtime/status` 必须保持边界清晰。 |
