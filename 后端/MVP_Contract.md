...
| --- | --- |
| 适用阶段 | MVP 首版上线 |
| 本版重点 | 增加管理员初始化冻结口径、邀请制接入生命周期、首登密码流程、post-login 幂等、runtime 跳转规则与 internal API 安全模型 |
...
- start → 注册 → post-login → membership 创建

### 2. 已登录用户接受邀请

- start → post-login → 直接绑定
