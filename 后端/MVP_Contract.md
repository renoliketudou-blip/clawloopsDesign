...
| --- | --- |
| 适用阶段 | MVP 首版上线 |
| 本版重点 | 增加站内登录与 invitation 首设密码流程、统一 session 鉴权边界、删除外部 IAM 依赖、补充种子管理员首登强制改密，并把 runtime 收敛为后端渲染配置 + RM 执行启动，新增个人空间文档操作与用户文件存储管理功能 |
...
- `/files` 与 `/files/edit/:path` 仅允许已登录且 `allowed=true`
...
- `admin runtime` 启动时同样复制公共区域到容器内副本，副本改动不回写宿主机
- 宿主机公共区受影响仅允许“管理员在工作台管理入口”的覆盖上传/删除动作
